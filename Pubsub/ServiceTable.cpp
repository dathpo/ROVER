...
Service ServiceTable::getService(byte id) {
	if (_table.size() > id) {
		...
		static Service newService(0, "0", false);
		return newService;
	}
}
...
void ServiceTable::addService(Service s) {
	...
	if (_table.find(id) == _table.end()) { // Service is not yet in table
		...
		for (std::map<int, int>::iterator it = portMap.begin();
				it != portMap.end(); ++it) {
...
void ServiceTable::mergeTable(ServiceTable t) {
	...
	for (std::map<byte, Service>::iterator it = otherTable.begin();
			it != otherTable.end(); ++it) {
...
string ServiceTable::toString() {
	string str = "Table: ";
	for (std::map<byte, Service>::iterator it = _table.begin();
			it != _table.end(); ++it) {
